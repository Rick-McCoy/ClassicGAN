...
def main():
    ...
    with tf.name_scope('input_noises'):
        ...
        time_num = tf.placeholder(dtype=tf.int32, name='time_num')
        train = tf.placeholder(dtype=tf.bool, name='traintest')
        noise1 = tf.stack(values=[input_noise1 for i in range(time_num)], axis=0)
        noise1 = tf.stack(values=[noise1 for i in range(CHANNEL_NUM)], axis=0, name='noise1')
        noise2 = noise_generator(noise=input_noise2, length=time_num)
        noise2 = tf.stack(values=[noise2 for i in range(CHANNEL_NUM)], axis=0, name='noise2')
        noise3 = tf.stack(values=[noise3 for i in range(time_num)], axis=1, name='noise3')
    ...
    with tf.name_scope('input_real'):
        real_input = tf.placeholder(dtype=tf.float32, shape=[None, CLASS_NUM, INPUT_LENGTH, CHANNEL_NUM], name='real_input')
    ...
    with tf.name_scope('loss1'):
        ...
        interpolate = real_input + tf.multiple(diff, alpha)
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in tqdm(range(TOTAL_TRAIN_EPOCH)):
            ...
            for songnum, path in enumerate(tqdm(pathlist)):
                ...
                summary, _, loss_val_dis1 = sess.run([merged, dis1_train, loss_dis1], feed_dict={input_noise1: feed_noise1, input_noise2: feed_noise2, input_noise3: feed_noise3, input_noise4: feed_noise4, real_input: batch_input, time_num: batch_num, train: True})
                writer.add_summary(summary, train_count)
                train_count+=1
                for i in range(5):
                    summary, _, loss_val_gen1 = sess.run([merged, gen1_train, loss_gen1], feed_dict={input_noise1: feed_noise1, input_noise2: feed_noise2, input_noise3: feed_noise3, input_noise4: feed_noise4, real_input: batch_input, time_num: batch_num, train: True})
                    writer.add_summary(summary, train_count)
                    train_count+=1
                summary, _, loss_val_dis2 = sess.run([merged, dis2_train, loss_dis2], feed_dict={input_noise1: feed_noise1, input_noise2: feed_noise2, input_noise3: feed_noise3, input_noise4: feed_noise4, real_input: batch_input, time_num: batch_num, train: True})
                writer.add_summary(summary, train_count)
                train_count+=1
                for i in range(5):
                    summary, _, loss_val_gen1 = sess.run([merged, gen1_train, loss_gen1], feed_dict={input_noise1: feed_noise1, input_noise2: feed_noise2, input_noise3: feed_noise3, input_noise4: feed_noise4, real_input: batch_input, time_num: batch_num, train: True})
                ...
                if songnum % 500 == 0:
                    batch_num = 15
                    samples = sess.run([gen3], feed_dict={input_noise1: feed_noise1, input_noise2: feed_noise2, input_noise3: feed_noise3, input_noise4: feed_noise4, real_input: batch_input, time_num: batch_num, train: True})

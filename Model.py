...
def generator3(inputs, encode, num, train):
    with tf.variable_scope('Generator3_' + str(num)):
        ...
        deconv2 = convolution(inputs=deconv1, filters=16, kernel_size=[1, 1, 2], strides=(1, 1, 2), training=train, regularization='batch_norm', transpose=True, name='deconv2')
...
def discriminator1(inputs, train, name):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        conv1 = convolution(inputs=inputs, filters=16, kernel_size=[2, 1, 1], strides=(2, 1, 1), training=train, name='conv1')
        # shape: [None, 16, 2, CLASS_NUM // 4, INPUT_LENGTH // 16]
        conv2 = convolution(inputs=conv1, filters=32, kernel_size=[2, 1, 1], strides=(2, 1, 1), training=train, name='conv2')
        # shape: [None, 32, 1, CLASS_NUM // 4, INPUT_LENGTH // 16]
        conv3 = convolution(inputs=conv2, filters=64, kernel_size=[1, 3, 1], strides=(1, 3, 1), training=train, name='conv3')
...
def discriminator2(inputs, train, name):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        conv1 = convolution(inputs=inputs, filters=16, kernel_size=[2, 1, 1], strides=(2, 1, 1), training=train, name='conv1')
        # shape: [None, 16, 2, CLASS_NUM // 2, INPUT_LENGTH // 8]
        conv2 = convolution(inputs=conv1, filters=32, kernel_size=[2, 1, 1], strides=(2, 1, 1), training=train, name='conv2')
        # shape: [None, 32, 1, CLASS_NUM // 2, INPUT_LENGTH // 8]
        conv3 = convolution(inputs=conv2, filters=64, kernel_size=[1, 6, 1], strides=(1, 6, 1), training=train, name='conv3')
        # shape: [None, 64, 1, CLASS_NUM // 12, INPUT_LENGTH // 8]
...
def discriminator3(inputs, train, name):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        conv1 = convolution(inputs=inputs, filters=16, kernel_size=[2, 1, 1], training=train, strides=(2, 1, 1), name='conv1')
        # shape: [None, 16, 2, CLASS_NUM, INPUT_LENGTH // 4]
        conv2 = convolution(inputs=conv1, filters=32, kernel_size=[2, 1, 1], training=train, strides=(2, 1, 1), name='conv2')
        # shape: [None, 32, 1, CLASS_NUM, INPUT_LENGTH // 4]
        conv3 = convolution(inputs=conv2, filters=64, kernel_size=[1, 6, 1], training=train, strides=(1, 6, 1), name='conv3')
        # shape: [None, 64, 1, CLASS_NUM // 6, INPUT_LENGTH // 4]
...
def discriminator4(inputs, train, name):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        conv1 = convolution(inputs=inputs, filters=16, kernel_size=[3, 1], training=train, strides=(6, 1), name='conv1')
        # shape: [None, 16, CLASS_NUM // 3, INPUT_LENGTH]
        conv2 = convolution(inputs=conv1, filters=32, kernel_size=[4, 1], training=train, strides=(12, 1), name='conv2')
        # shape: [None, 32, CLASS_NUM // 12, INPUT_LENGTH]
        conv3 = convolution(inputs=conv2, filters=64, kernel_size=[6, 1], training=train, strides=(1, 2), name='conv3')
        # shape: [None, 64, 1, INPUT_LENGTH]
        conv4 = convolution(inputs=conv3, filters=128, kernel_size=[1, 2], training=train, strides=(1, 2), name='conv4')
        # shape: [None, 128, 1, INPUT_LENGTH // 2]
        conv5 = convolution(inputs=conv4, filters=128, kernel_size=[1, 4], training=train, strides=(1, 2), name='conv5')
        # shape: [None, 128, 1, INPUT_LENGTH // 8]
        conv6 = convolution(inputs=conv5, filters=256, kernel_size=[1, 4], training=train, strides=(1, 4), name='conv6')
        # shape: [None, 256, 1, INPUT_LENGTH // 32]
        conv7 = convolution(inputs=conv6, filters=512, kernel_size=[1, 4], training=train, strides=(1, 4), name='conv7')

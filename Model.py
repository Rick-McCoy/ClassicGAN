...
def generator(noise, sharednoise, num, train):
    with tf.variable_scope('generator' + str(num)):
        ...
        deconv1 = tf.layers.conv2d_transpose(inputs=dropout2, filters=64, kernel_size=[4, 8], strides=(2, 2), padding='same', activation=leaky_relu, name='deconv1', bias_initializer=None)
        batch_norm1 = tf.layers.batch_normalization(inputs=deconv1)
        deconv2 = tf.layers.conv2d_transpose(inputs=batch_norm1, filters=32, kernel_size=[4, 8], strides=(2, 2), padding='same', activation=leaky_relu, name='deconv2', bias_initializer=None)
        ...
        deconv5 = tf.layers.conv2d_transpose(inputs=batch_norm4, filters=1, kernel_size=[12, 8], strides=(1, 2), padding='same', activation=None, name='deconv5', bias_initializer=None)
